let t=[],e=1,o=1;const i=document.querySelector(".product__categories-box"),a=document.querySelector(".product__categories-menu"),r=document.querySelector(".product__categories-text"),s=document.querySelector(".product__atoz-box"),d=document.querySelector(".product__atoz-menu"),n=document.querySelector(".product__atoz-text"),l=document.querySelector(".product__search-input"),c=document.getElementById("products-container"),p=document.querySelector(".pagination"),u=p.querySelector(".first-page"),_=p.querySelector(".prev-page"),v=p.querySelector(".page-numbers"),C=p.querySelector(".next-page"),g=p.querySelector(".last-page");function h(t){t.style.display="block"===t.style.display?"none":"block"}function m(t,o,i){t.querySelectorAll(".product__dropdown-item").forEach(t=>{t.classList.remove("active"),t.style.color="rgba(1,1,1,0.5)"}),i.classList.add("active"),i.style.color="#010101",o.textContent=i.textContent,t.style.display="none",e=1,w()}async function f(e=1){try{let i=await fetch(`https://food-boutique.b.goit.study/api/products?page=${e}&limit=1000`);t=(await i.json()).results||[],o=Math.ceil(t.length/9),r.textContent&&"Categories"!==r.textContent||(r.textContent="Show all"),w()}catch(t){console.error("Error fetching products:",t)}}function w(){let i=[...t],a=(l?.value||"").trim().toLowerCase();a&&(i=i.filter(t=>(t.name||"").toLowerCase().includes(a)));let s=(r.textContent||"").trim();s&&"Show all"!==s&&(i=i.filter(t=>t.category&&t.category.replace(/_/g," ")===s));let d=(n.textContent||"").trim();"A to Z"===d?i.sort((t,e)=>t.name.localeCompare(e.name)):"Z to A"===d?i.sort((t,e)=>e.name.localeCompare(t.name)):"Cheap"===d?i.sort((t,e)=>t.price-e.price):"Expensive"===d?i.sort((t,e)=>e.price-t.price):"Popular"===d?i.sort((t,e)=>e.popularity-t.popularity):"Not popular"===d&&i.sort((t,e)=>t.popularity-e.popularity),o=Math.ceil(i.length/9);let p=(e-1)*9;(function(t){let e=document.querySelector(".pagination");c.classList.add("fade-out"),setTimeout(()=>{if(c.innerHTML="",!t||0===t.length){c.innerHTML=`
<div class="product__nothing-list">
  ...
  <p class="product__nothing-found-dex">
    ...
    to find the perfect product for you.
      ...
      `,e.classList.add("pagination__hidden"),c.classList.remove("fade-out"),c.classList.add("fade-in");return}t.forEach(t=>{let e=document.createElement("div");e.classList.add("product-card"),e.innerHTML=`
        <div class="product-card__bg"></div>
        <div class="product-card__image-bg"></div>
        <img class="product-card__image" src="${t.img}" alt="${t.name}" />
        <div class="product-card__info">
          <div class="product-card__info-name">${t.name}</div>
          <div class="product-card__info-details">
            <div class="product-card__info-details-row">
              <div class="product-card__info-details-row-item">
                ...
                <div class="product-card__info-details-row-item-value">${(t.category||"").replace(/_/g," ")}</div>
              </div>
              <div class="product-card__info-details-row-item">
                <div class="product-card__info-details-row-item-label">Size:</div>
                <div class="product-card__info-details-row-item-value">${t.size??""}</div>
              </div>
            </div>
            <div class="product-card__info-details-row">
              <div class="product-card__info-details-row-item">
                <div class="product-card__info-details-row-item-label">Popularity:</div>
                <div class="product-card__info-details-row-item-value">${t.popularity??""}</div>
              </div>
            </div>
          </div>
        </div>
        <div class="product-card__footer">
          <div class="product-card__footer-price">$${t.price}</div>
 ...
 <button class="product-card__footer-button-icon">
  ...
  >
    ...
    />
      ...
      `,c.appendChild(e)}),e.style.display="",c.classList.remove("fade-out"),c.classList.add("fade-in"),e.classList.remove("pagination__hidden")},250)})(i.slice(p,p+9)),function(){v.innerHTML="";let t=Math.max(1,e-1),i=Math.min(o,t+3);i-t<3&&(t=Math.max(1,i-3));for(let o=t;o<=i;o++){let t=document.createElement("button");t.classList.add("pagination__page-btn"),t.textContent=o,o===e&&t.classList.add("pagination__page-btn--active"),t.onclick=()=>y(o),v.appendChild(t)}}()}function y(t){t<1||t>o||t===e||(e=t,w())}i.addEventListener("click",t=>{t.stopPropagation(),h(a),d.style.display="none"}),s.addEventListener("click",t=>{t.stopPropagation(),h(d),a.style.display="none"}),a.querySelectorAll(".product__dropdown-item").forEach(t=>{t.addEventListener("click",t=>m(a,r,t.target))}),d.querySelectorAll(".product__dropdown-item").forEach(t=>{t.addEventListener("click",t=>m(d,n,t.target))}),document.addEventListener("click",()=>{a.style.display="none",d.style.display="none"}),l&&l.addEventListener("input",()=>{e=1,w()}),[u,_,C,g].forEach(t=>{t.classList.add("pagination__btn")}),u.innerHTML=`
 ...
 <svg xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
...
</svg>`,u.onclick=()=>y(1),_.onclick=()=>y(Math.max(1,e-1)),C.onclick=()=>y(Math.min(o,e+1)),g.onclick=()=>y(o),f(e);const L=document.querySelector(".popular__list");fetch("https://food-boutique.b.goit.study/api/products/popular").then(t=>t.json()).then(t=>{t.forEach(t=>{let e=document.createElement("div");e.classList.add("popular__item"),e.innerHTML=`
        <div class="popular__item-background"></div>
        <div class="popular__item-content">
          <div class="popular__image-wrapper"></div>
          <img class="popular__image" src="${t.img}" alt="${t.name}" />
          <div class="popular__info">
            <div class="popular__name">${t.name}</div>
            <div class="popular__details">
              <div class="popular__detail"><div class="popular__label">Category:</div><div class="popular__value">${t.category}</div></div>
              <div class="popular__detail"><div class="popular__label">Size:</div><div class="popular__value">${t.size}</div></div>
              <div class="popular__detail"><div class="popular__label">Popularity:</div><div class="popular__value">${t.popularity}</div></div>
            </div>
          </div>
  ...
  >
    ...
    />
      ...
      `,L.appendChild(e)})});const k=document.querySelector(".discount__list");fetch("https://food-boutique.b.goit.study/api/products/discount").then(t=>t.json()).then(t=>{let e=JSON.parse(localStorage.getItem("discountRemaining"))||[...t];e.length<2&&(e=[...t]),e.sort(()=>.5-Math.random());let o=e.slice(0,2);e=e.slice(2),localStorage.setItem("discountRemaining",JSON.stringify(e)),k.innerHTML="",o.forEach(t=>{let e=document.createElement("div");e.classList.add("discount__item"),e.innerHTML=`
        <div class="discount__background"></div>
        <div class="discount__image-bg"></div>
        <img class="discount__image" src="${t.img}" alt="${t.name}" />
        <div class="discount__name">${t.name}</div>
        <div class="discount__price">$${t.price}</div>
